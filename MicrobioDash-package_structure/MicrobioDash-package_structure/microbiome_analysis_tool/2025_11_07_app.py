        ...
        try:
            ...
            if not factors and not interactions:
                permanova_res = html.Div([
                    ...
                    "border": "3px solid #e74c3c", "padding": "18px", "borderRadius": "10px",
                    "background": "#fadbd8", "fontSize": "1.1em"
                })
            ...
            # 5. Berechnung
                ...
                # R² Plot
                r2_fig = px.bar(
                    mf_res[['treatment', 'R²']].replace(':', ' × ', regex=True),
                    x='term', y='R2', title="explained variance (R²)",
                    color='R2', color_continuous_scale='Blues', text='R2'
                )
                ...
                try:
                    ...
                    disp = permdisp(dm_sk, meta_rel[treat_col].astype(str))
                    disp_text = f"F = {disp['test statistic']:.2f}, p = {disp['p-value']:.3f}"
                except:
                ...
                permanova_res = html.Div([
                    ...
                    html.H5("PERMDISP"),
                    html.P(disp_text, style={"background": "#ecf0f1", "padding": "8px", "fontFamily": "monospace"}),
                    html.P("permanova_results.csv", style={"color": "#27ae60", "fontWeight": "bold"})
                ], style={
                    "border": "3px solid #27ae60", "padding": "20px", "borderRadius": "12px",
                    "background": "#d5f4e6", "boxShadow": "0 4px 12px rgba(0,0,0,0.1)"
                })

        except Exception as e:
            logger.error(f"PERMANOVA Fehler: {e}")
            permanova_res = html.Div([
                html.H4("PERMANOVA Fehler", style={"color": "#c0392b"}),
                html.Pre(str(e)[:300])
            ], style={"border": "2px solid #c0392b", "padding": "15px", "background": "#fadbd8"})
    
        except ValueError as ve: # Catch the specific ValueError we raised for skipping
            permanova_res = html.Div(
                [
            ...
            logger.info(f"PERMANOVA skipped: {ve}")
            ...
            permanova_res = html.Div(
                [
                    ...
                    html.Pre(str(e_perm), style={'whiteSpace': 'pre-wrap', 'color': '#b00'})
                ],
                style={
