...
def permanova_marginal(
    ...
    logger.info(f"Unique levels for Cultivar: {metadata_df['Cultivar'].unique()}")
    logger.info(f"Number of unique levels for Cultivar: {metadata_df['Cultivar'].nunique()}")
    
    logger.info(f"Unique levels for Substrate: {metadata_df['Substrate'].unique()}")
    logger.info(f"Number of unique levels for Substrate: {metadata_df['Substrate'].nunique()}")
    
    # Überprüfen Sie alle einzigartigen Kombinationen der Interaktion
    interaction_combinations = metadata_df.groupby(['Cultivar', 'Substrate']).size().reset_index(name='count')
    logger.info(f"Actual Cultivar:Substrate combinations in data:\n{interaction_combinations}")
    logger.info(f"Number of actual Cultivar:Substrate combinations: {len(interaction_combinations)}")
    ...
    logger.info(f"Patsy X_full shape: {X_full.shape}")
    logger.info(f"Patsy df_per_term: {df_per_term}")
    logger.info(f"Patsy term_slices: {term_slices}")
    if X_full.shape[1] == 0:
